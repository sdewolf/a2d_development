...
// modified: Thursday, September 29, 2017 (2017272)
...
int main()
{
  ...
  sprintf(path, "/home/lab2/LabJack/data/%4i", ft.tm_year+1900);
  if (stat(path, &st) == -1)
    mkdir(path, 0755);
  ...
  sprintf(path, "/home/lab2/LabJack/data/%4i/%03i", ft.tm_year+1900, ft.tm_yday+1);
  if (stat(path, &st) == -1)
    mkdir(path, 0755);
